...
const TopTabsBar = styled.div`
  ...
  padding: 0 20px 0;          /* 🔸 위/아래 패딩 0, 양 옆만 */
  background: #fffcf4;
  position: fixed;
  top: 50px;
  left: 0;
...
const TopTabsInner = styled.div`
  ...
  padding-top: 10px;          /* 🔸 실제 탭 아이콘 위 여백은 여기서만 살짝 */
`;
...
const MobileFloatingBar = styled.div`
  ...
  /* 화면 아래에서 조금 위로 띄워서 항상 보이게 */
  bottom: 120px;

  padding: 0 16px;
  background: transparent; /* 바 전체 배경은 없애고 버튼만 보이게 */
  box-shadow: none;
  display: flex;
...
const MobileFloatingButton = styled.button`
  min-width: 120px;          /* 전체 폭 차지 X, 작은 pill 버튼 */
  height: 40px;
  padding: 0 16px;
...
function ProgramDetail({ program }) {
  ...
  const handleScrollToBooking = () => {
    ...
    // 상단 탭/헤더 높이만큼 조금 위로 여유 (대략 120px)
