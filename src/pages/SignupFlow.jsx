...
const Card = styled.div`
  ...
  box-shadow: 0 14px 36px rgba(0,0,0,.06); padding: 32px 28px;
`;
const Head = styled.div`
  margin-bottom: 28px;
  h1 { margin: 0 0 10px; font-size: clamp(22px, 3.2vw, 36px); color: #1b2b3a; letter-spacing: -0.3px; }
...
`;
const SectionTitle = styled.h3`margin:0 0 12px; color:#1b2b3a; font-size:18px;`;
const ButtonRow = styled.div`
  display: flex; justify-content: center; align-items: center; margin-top: 24px;
`;
const BaseBtn = styled.button`
  width: 260px; height: 56px; border: 0; border-radius: 16px;
  font-size: 16px; display: inline-flex; align-items: center; justify-content: center;
  box-shadow: 0 10px 24px rgba(0,0,0,.06); cursor: pointer; transition: filter .12s, transform .04s;
  &:hover { filter: brightness(0.98); }
  &:active { transform: translateY(1px); }
`;
const PhoneBtn = styled(BaseBtn)`
  background: linear-gradient(180deg, #F2F3F5 0%, #EDEFF2 100%);
  color: #111; border: 1px solid rgba(0,0,0,0.06);
  box-shadow:
...
export default function SignupFlow() {
  ...
  const handleVerified = useCallback(async (phoneE164) => {
    ...
    const memberSkeleton = {
      ...
      memberships: { agitz: null, agitzList: [], family: null, timepass: null, points: null },
    ...
    try {
      ...
      try { localStorage.setItem("withagit.memberdata", JSON.stringify(memberSkeleton)); } catch { }
    } catch (e) {
      // 업서트 실패해도 플로우는 진행 (완료 페이지/마이페이지에서 재시도)
      console.warn("[signup] upsertMemberByPhone failed (will fallback later)", e);
    }
  ...
  return (
    <Page>
      <Card>
        <Head>
          <h1>간편 회원가입</h1>
          <p>소중한 회원님의 정보는 개인정보보호정책에 의거하여 보호받고 있습니다.</p>
        </Head>
