...
export default function BranchesPage() {
  ...
  return (
    <Page>
          ...
          <>
            <Grid>
              {branches.map((b) => {
                if (b.type === "open") {
                  return (
                    <Card key={b.id} to={`/branches/${b.id}`}>
                      <Thumb><img src={b.thumbnail} alt={`${b.name} 썸네일`} loading="lazy" /></Thumb>
                      <CardBody>
                        <CardTitle>{b.name}</CardTitle>
                        <CardSub>{b.district}</CardSub>
                      </CardBody>
                    </Card>
                  );
                }
                if (b.type === "coming") {
                  return (
                    <SpecialCard key={b.id}>
                      <SpecialBody>
                        <h3>{b.title}</h3>
                        <p style={{ marginTop: 6, fontWeight: 900 }}>{b.subtitle}</p>
                      </SpecialBody>
                      <SpecialFooter>
                        <CardTitle>{b.footTitle}</CardTitle>
                        <CardSub>{b.footSub}</CardSub>
                      </SpecialFooter>
                    </SpecialCard>
                  );
                }
                ...
                return (
                  <SpecialCard key={b.id}>
                    <SpecialBody>
                      ...
                      <p>{b.subtitle}</p>
                      {b.ctaLabel && (
                        <CtaBtn type="button" onClick={() => setOpenNext(true)} style={{ marginTop: 12 }}>
                          {b.ctaLabel}
                        </CtaBtn>
                      )}
                    </SpecialBody>
                    <SpecialFooter>
                      <CardTitle>우리 동네에 아지트가 필요하세요?</CardTitle>
                      <CardSub>집·학교·놀이터가 있는 생활권을 알려주세요.</CardSub>
                    </SpecialFooter>
                  </SpecialCard>
                );
              })}
      ...
      {/* ✅ 다음 아지트 제안 모달 */}
      <NextAgitModal open={openNext} onClose={() => setOpenNext(false)} onSubmitted={() => { }} />
    </Page>
  );
}
