...
export default function CheckoutConfirmDialog({
    ...
    const subtotal = isPoints ? pointsAmount : baseSubtotal;
    const discount = isPoints ? 0 : baseDiscount;
    const total = isPoints ? pointsAmount : (baseSubtotal - baseDiscount);
    ...
    const readActiveSet = useCallback(async (phoneE164, kindName) => {
        ...
        const qy = query(col, where("kind", "==", kindName), where("status", "in", ["active", "future"]));
    ...
    // 초기 선택: 항상 첫 자녀(AGITZ/FAMILY는 가급적 미적용 자녀)
    useEffect(() => {
        const list = childrenForUI || [];
        if (!list.length) { setSelectedChildId(""); return; }
        if (kind === MEMBERSHIP_KIND.AGITZ) {
            const first = list.find(c => !agitzAppliedSet.has(c.childId)) || list[0];
    ...
    const canPay = useMemo(() => {
        ...
        // 중복 가드
        if (kind === MEMBERSHIP_KIND.AGITZ && agitzAppliedSet.has(selectedChildId)) return false;
        if (kind === MEMBERSHIP_KIND.FAMILY && familyAppliedSet.has(selectedChildId)) return false;
        // TIMEPASS 분 유효성
        if (kind === MEMBERSHIP_KIND.TIMEPASS) {
            const m = resolveTimepassMinutes(product, payload); if (!(m > 0)) return false;
        }
        return true;
    }, [agree, loading, selectedChildId, total, kind, agitzAppliedSet, familyAppliedSet, product, payload]);

    const fail = (code, msg) => ({ ok: false, code, message: msg });

    // === DB 스냅샷 기반 사전검증 (모든 타입에 childId 요구) ===
    const preflightValidateDb = useCallback(() => {
        ...
        if (!selectedChildId) return fail("child_not_selected", "적용할 자녀를 선택해 주세요.");
        if (!childrenForUI.find(c => c.childId === selectedChildId)) return fail("child_not_owned", "선택한 자녀를 찾을 수 없습니다.");

        if (kind === MEMBERSHIP_KIND.AGITZ && agitzAppliedSet.has(selectedChildId)) {
            return fail("agitz_child_already_applied", "이미 정규 멤버십이 적용된 자녀입니다.");
        }
        if (kind === MEMBERSHIP_KIND.FAMILY && familyAppliedSet.has(selectedChildId)) {
            return fail("family_child_already_applied", "이미 패밀리 멤버십이 적용된 자녀입니다.");
        }
        if (kind === MEMBERSHIP_KIND.TIMEPASS) {
        ...
        childrenForUI, selectedChildId, agitzAppliedSet, familyAppliedSet,
        product, payload
    ]);
    ...
    function buildDraft(rawE164) {
        ...
        const draft = {
            type, // ORDER_TYPE.*
            childId: selectedChildId || null, // ✅ 전 타입 공통으로 childId 포함(POINTS도 메타용 포함)
            months:
    ...
    const handlePay = async () => {
        ...
        try {
            ...
            if (typeof onCreateOrder === "function") {
                orderRes = await onCreateOrder(draft); // 상위에서 createOrderDraft 호출 권장
            } else {
            ...
            const response = await Bootpay.requestPayment({
                ...
                items: [{ id: product?.id || kind, name: product?.name || kind, qty: 1, price: Number(isPoints ? pointsAmount : total) || 0 }],
                metadata: sanitizeForFirestore({ productId: product?.id || "", kind, variant: product?.variant || "", childId: selectedChildId }),
                extra: {
                    open_type: "iframe",
                    browser_open_type: [
        ...
        return (
            <>
                <div style={{ display: "grid", gap: 8 }}>
                    {childrenForUI.map((c) => {
                        const isAgitz = agitzAppliedSet.has(c?.childId);
                        const isFamily = familyAppliedSet.has(c?.childId);
                        const disable =
                            (kind === MEMBERSHIP_KIND.AGITZ && isAgitz) ||
                            (kind === MEMBERSHIP_KIND.FAMILY && isFamily);

                        let statusBadge = null;
                        if (isAgitz && kind === MEMBERSHIP_KIND.AGITZ) statusBadge = <Badge>정규 가입됨</Badge>;
                        else if (isFamily && kind === MEMBERSHIP_KIND.FAMILY) statusBadge = <Badge>패밀리 가입됨</Badge>;
                        else if (isAgitz && kind !== MEMBERSHIP_KIND.AGITZ) statusBadge = <Badge>정규 가입됨</Badge>;
                        else if (isFamily && kind !== MEMBERSHIP_KIND.FAMILY) statusBadge = <Badge>패밀리 가입됨</Badge>;
                            ...
                            >
                                ...
                                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                    <input
                                        ...
                                        name="child_pick"
                                        disabled={disable}
                                        checked={selectedChildId === c?.childId}
                                        onChange={() => setSelectedChildId(c?.childId)}
                                    />
                                </div>
                            </div>
                        );
                ...
                <Dialog role="dialog" aria-modal="true">
                    ...
                    <Body>
                        ...
                        <AmountBox>
                            ...
                            <div className="line"><span>상품금액</span><span className="v">{KRW(subtotal)}원</span></div>
